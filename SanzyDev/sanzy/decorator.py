...
from .tools import sanzyChanger
    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            sanzy_reg = sudo_reg = re.compile(pattern)
        else:
            sanzy_ = "\\" + var.CMD_HANDLER
            sudo_ = "\\" + var.SUDO_HANDLER
            sanzy_reg = re.compile(sanzy_ + pattern)
                ...
                if not disable_edited:
                    sanzy.add_event_handler(
                        ...
                        events.MessageEdited(
                            **args,
                            from_users=sanzyChanger(sudoer()),
                            pattern=sudo_reg
                        ),
                    )
                sanzy.add_event_handler(
...
def chataction(**args):
    def decorator(func):
        if sanzy:
            sanzy.add_event_handler(func, events.ChatAction(**args))
        return func
