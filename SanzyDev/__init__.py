...
if var.STRING_SESSION:
    ...
    session = "SanzyUserBot"
try:
    Sanzy = TelegramClient(
        session=session,
        api_id=var.API_KEY,
...
ch = str(b64decode("QFNhbnp5X3VwZGF0ZWJvdA=="))[2:15]
gc = str(b64decode("QFNhbnp5Q2hhdHM="))[2:13]
...
async def update_restart_msg(chat_id, msg_id):
    ...
    message = (
        f"**Sanzy-UserBot v`{var.BOT_VER}` is back up and running!**\n\n"
        f"**Telethon:** `{vsc}`\n"
        f"**Python:** `{python_version()}`\n"
    )
    await Sanzy.edit_message(chat_id, msg_id, message)
    return True
