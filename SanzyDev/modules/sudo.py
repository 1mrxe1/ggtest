...
from SanzyDev.database.sudoer import add_sudo, del_sudo, sudoer

from . import cmd


@sanzy_cmd(pattern="sudo$")
async def sudo(event):
    listsudo = SanzyChanger(sudoer())
    text = "Daftar sudo\n\n"
    no = 0
    if listsudo:
...
@sanzy_cmd(pattern="addsudo(?:\\s|$)([\\s\\S]*)", allow_sudo=False)
async def add(event):
    suu = event.text[9:]
    if f"{cmd}add " in event.text:
        return
    sudo = SanzyChanger(sudoer())
...
@sanzy_cmd(pattern="delsudo(?:\\s|$)([\\s\\S]*)", allow_sudo=False)
async def _(event):
    suu = event.text[8:]
    sudo = SanzyChanger(sudoer())
