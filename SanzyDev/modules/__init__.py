...
def __list_all_modules():
    ...
    if var.LOAD or var.NO_LOAD:
        ...
        if to_load:
            ...
            to_load = all_modules
        if var.NO_LOAD:
            LOGS.info("Modules No Load : {}".format(var.NO_LOAD))
            return [item for item in to_load if item not in var.NO_LOAD]
        return to_load
    return all_modules
