...
@sanzy_cmd(pattern="convert ?(foto|audio|gif|voice|photo|mp3)? ?(.*)")
    ...
    if botyins in ["foto", "photo"]:
        ...
        if efekt in EFEKTLER:
            ...
            ses = await asyncio.create_subprocess_shell(
                f"ffmpeg -i '{indir}' {KOMUT[efekt]} sanzy.mp3"
            )
        ...
        gif = await asyncio.create_subprocess_shell(
            f"ffmpeg -y -i '{video}' -vn -b:a 128k -c:a libmp3lame sanzy.mp3"
        )
