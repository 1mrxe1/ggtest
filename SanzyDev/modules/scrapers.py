...
from SanzyDev.sanzy.FastTelethon import upload_file
...
@sanzy_cmd(pattern="currency ([\\d\\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@sanzy_cmd(pattern=r"lang (tr|tts) (.*)")
...
@sanzy_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
...
async def kbg(remob):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "sanzy_bg.png"
...
async def ocr(event):
    ...
    downloaded_file_name = await sanzy.download_media(
...
@sanzy_cmd(pattern=r"makeqr(?: |$)([\s\S]*)")
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example :** `{cmd}google -l4 SanzyDev` atau `{cmd}google SanzyDev`\
    "
    }
)
