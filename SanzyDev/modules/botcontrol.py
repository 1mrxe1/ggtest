...
from SanzyDev import Sanzy, LOGS, StartTime, bot
from SanzyDev.database.bot_blacklist import check_is_black_list
from SanzyDev.database.bot_start import (
    add_starter_to_db,
    get_all_starters,
    get_starter_details,
)
from SanzyDev.database.variable import get_var
from SanzyDev.Sanzy import _format, asst_cmd, callback, reply_id, set_var_value
...
async def check_bot_started_users(user, event):
    if user.id == (await Sanzy.get_me()).id:
        return
...
async def pmclose(event):
    if event.query.user_id == (await Sanzy.get_me()).id:
        await event.delete()
...
async def pmbot(event):
    await event.delete()
    SanzyUBOT = await bot.get_me()
    botusername = SanzyUBOT.username
    if event.query.user_id == (await Sanzy.get_me()).id:
...
async def strone(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"**Silahkan Kirimkan {var} Telethon anda dari @SanzyStringRobot**\n\nGunakan /cancel untuk membatalkan."
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
    ...
    if chat.id != owner.id:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**👋 Hai** {mention}**!**\
                        ...
                        \n\n**Powered by** [UserBot](https://github.com/sanzydev/Sanzy-Userbot)"
        buttons = [
            (
