...
from SanzyDev import CMD_HELP, COUNT_PM, LASTMSG, LOGS, Sanzy
from SanzyDev.database.permit import (
    approve,
    del_permit_message,
    disapprove,
    get_permit_message,
...
from SanzyDev.database.variable import del_var, get_var, set_var
from SanzyDev.events import sanzy_cmd
from SanzyDev.sanzy import SanzyChanger, eod, eor
...
DEF_UNAPPROVED_MSG =  (
    """
🙋🏻‍♂️ ʜᴀʟᴏ  ᴀᴅᴀ ʏᴀɴɢ ʙɪsᴀ sᴀʏᴀ ʙᴀɴᴛᴜ?
...
@Sanzy.on(events.NewMessage(incoming=True))
async def permitpm(event):
    ...
    if (
        ...
        apprv = SanzyChanger(is_approved())
            ...
            if event.chat_id in DEVS:
                if event.chat_id not in apprv:
                    try:
                        ...
                        await sanzy.send_message(var.BOTLOG_CHATID, f"**#AUTO_APPROVED_DEVELOPER**\n\n👑 **Developer:** [{sender.first_name}](tg://user?id={sender.id})\n💬 `Developer sanzy-Userbot Telah Mengirimi Anda Pesan...`")
                        await sanzy.send_message(
                            event.chat_id,
                            f"**Menerima Pesan!!!**\n**Terdeteksi [{sender.first_name}](tg://user?id={sender.id}) Adalah Developer sanzy-Userbot**"
                        )
                        return
...
@Sanzy.on(events.NewMessage(outgoing=True))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"notifoff$"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"notifon$"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"block$"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"unblock$"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"permit (on|off)(?: |$)(\w*)"))
...
@Sanzy.on(sanzy_cmd(outgoing=True, pattern=r"(set|get|reset) pmpermit(?: |$)(\w*)"))
