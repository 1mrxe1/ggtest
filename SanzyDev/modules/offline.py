...
from SanzyDev import AFKREASON
from SanzyDev.sanzy import sanzyChanger, sanzy_cmd, sanzy_handler
from SanzyDev.database.permit import is_approved, get_mode_permit
...
@sanzy_handler(incoming=True, func=lambda e: e.is_private)
async def afk_on_pm(sender):
    ...
    global afk_end
    back_alivee = datetime.now()
    sanzyBot = await sender.client.get_me()
    owner = sanzyBot.first_name
    afk_end = back_alivee.replace(microsecond=0)
    ...
    if (
        ...
        is_app = sanzyChanger(is_approved())
        if get_mode_permit():
