...
from SanzyDev.database.muted import (
    add_mute,
    add_gmute,
    cek_mute,
    cek_gmute,
...
@sanzy_cmd(pattern="setgpic( -s| -d)$", group_only=True)
...
@sanzy_cmd(pattern="promote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@sanzy_cmd(pattern="demote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@sanzy_cmd(pattern="ban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
async def ban(bon):
    ...
    if not user:
        return
    sanzy = await eor(bon, "`Pemrosesan Larangan...`")
    try:
...
@sanzy_cmd(pattern="unban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@sanzy_cmd(pattern="mute(?: |$)(.*)", group_only=True)
...
async def spider(spdr):
    ...
    muting = sanzyChanger(cek_mute(self_user.id))
...
@sanzy_cmd(pattern="unmute(?: |$)(.*)", group_only=True)
...
@sanzy_cmd(pattern="ungmute(?: |$)(.*)", group_only=True)
...
async def ungmoot(un_gmute):
    ...
    sanzy = await eor(un_gmute, '**Memproses...**')
    user = await get_user_from_event(un_gmute)
    user = user[0]
    if not user:
        return
    await sanzy.edit("**Membuka Global Mute Pengguna...**")
    ungmuting = sanzyChanger(cek_gmute(me.id))
    if user.id in ungmuting:
        await sanzy.edit("**KESALAHAN!** Pengguna Sudah Tidak Di Bisukan Global...")
        del_gmute(me.id, user.id)
        return
    else:
        await sanzy.edit("**BERHASIL! Pengguna Sudah Tidak Dibisukan**", time=10)


@sanzy_cmd(pattern="gmute(?: |$)(.*)", group_only=True)
...
async def gspider(gspdr):
    try:
        ...
        sanzy = await eor(gspdr, '**Memproses...**')
        user, reason = await get_user_from_event(gspdr)
        if not user:
            return
        self_user = await gspdr.client.get_me()
        if user.id == self_user.id:
            return await sanzy.edit("**Tidak Bisa Membisukan Diri Sendiri..（>﹏<）**")
        if user.id in DEVS:
            return await sanzy.edit("**Gagal Membisukan Global, Karna dia adalah Pembuat Saya 🤪**")
        await sanzy.edit("**Berhasil Membisukan Pengguna!**")
        gmuted = sanzyChanger(cek_gmute(self_user.id))
...
'''
        ...
        await eor(sanzy, f'{format_exc()}')


@sanzy_cmd(pattern="zombies(?: |$)(.*)", group_only=True)
...
@sanzy_cmd(pattern="admins$", group_only=True)
...
@sanzy_cmd(pattern="kick(?: |$)(.*)", group_only=True)
