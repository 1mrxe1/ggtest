...
@sanzy_cmd(pattern="pdf(?: |$)(.*)")
async def _(event):
    if not event.reply_to_msg_id:
        return await eor(event, "**Mohon Reply ke teks apa pun**")
    reply_message = await event.get_reply_message()
    chat = "@office2pdf_bot"
    xx = await eor(event, "`Mengubah menjadi PDF...`")
    try:
        async with event.client.conversation(chat) as conv:
            try:
                ...
                filename = await conv.send_message("Sanzy-Userbot")
