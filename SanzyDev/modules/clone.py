...
from SanzyDev import CMD_HELP, LOGS, STORAGE
from SanzyDev.sanzy import eor
...
@sanzy_cmd(pattern="clone ?(.*)", allow_sudo=False)
async def impostor(event):
    inputArgs = event.pattern_match.group(1)
    SanzyDev = ["@sanzydev", "@sanzydevv"]
    if inputArgs in SanzyDev:
        await eor(event, "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer sanzy-Userbot Ngentod 😡")
        await event.client.send_message(
            "@SanzyChats",
            "**Maaf Telah MengClone Sanzy 🥺**"
        )
        return
    ...
    if inputArgs:
        ...
        if replyMessage.sender_id in DEVS:
            return await xx.edit(
                "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Sanzy-Userbot Ngentod 😡"
            )
