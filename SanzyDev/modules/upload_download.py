...
from SanzyDev import CMD_HELP, LOGS
from SanzyDev.sanzy import sanzy_cmd, eor, humanbytes, progress, run_cmd
from SanzyDev.sanzy.FastTelethon import download_file, upload_file

from . import cmd, var


@sanzy_cmd(pattern="download(?: |$)(.*)")
async def download(target_file):
    """For .download command, download files to the userbot's server."""
    xx = await eor(target_file, "**Memprosess...**")
    ...
    if input_str:
        ...
        if downloader.isSuccessful():
            await xx.edit(f"Berhasil diunduh ke `{downloaded_file_name}`!!!"
            )
        else:
            await xx.edit(f"URL salah\n{url}")
    elif replied:
        if not replied.media:
            return await xx.edit("`Balas ke file atau media`")
        try:
            media = replied.media
            if hasattr(media, "document"):
                    ...
                    if "audio" in mime_type:
                        ...
                        filename = (
                            ...
                            datetime.now().isoformat(
                                ...
                                "seconds") +
                            ".mp4")
                outdir = var.TEMP_DOWNLOAD_DIRECTORY + filename
                c_time = time.time()
                ...
                result = await target_file.client.download_media(
                    media, var.TEMP_DOWNLOAD_DIRECTORY
                )
        ...
        await xx.edit(f"Ketik `{cmd}help download` untuk bantuan menggunakan module.")
    ...
    if os.path.exists(input_str):
        if os.path.isfile(input_str):
            ...
            if thumb is not None:
                os.remove(thumb)
            await xx.edit(f"Berhasil diunggah dalam `{up_time}` detik.")
            ...
            await xx.edit(f"Menemukan file `{len(lst_files)}`. Sekarang sedang diunggah...")
            for files in os_sorted(lst_files):
                file_name = os.path.basename(files)
                thumb = None
                attributes = []
                msg = await xx.edit(f"Mengunggah `{files}`")
            ...
            up_time = (datetime.now() - start_time).seconds
            await xx.respond(
                f"Mengunggah file `{len(lst_files)}` di folder `{input_str}` \ndalam `{up_time}` detik."
            )
    else:
        await xx.edit("`404: File/Folder Tidak Ditemukan`")
