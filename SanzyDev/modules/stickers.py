...
from SanzyDev import bot
from SanzyDev.sanzy import sanzy_cmd, eod, eor
from SanzyDev.sanzy.misc import animator, create_quotly
from SanzyDev.database.variable import cek_var, get_var, set_var
...
@sanzy_cmd(pattern="(?:tikel|kang)\\s?(.)?")
async def kang(args):
...
@sanzy_cmd(pattern="pkang(?:\\s|$)([\\s\\S]*)")
async def _(event):
    xnxx = await eor(event, f"`{choice(KANGING_STR)}`")
    reply = await event.get_reply_message()
    query = event.text[7:]
    sanzyBot = await bot.get_me()
    BOT_USERNAME = sanzyBot.username
    bot_ = BOT_USERNAME
    bot_un = bot_.replace("@", "")
        ...
        try:
            create_st = await bot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
                    title=pname,
                    short_name=f"sanzy_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
@sanzy_cmd(pattern="stickers ?([\\s\\S]*)")
