...
from SanzyDev.sanzy import HOSTED_ON, sanzy_cmd, eod, eor, install_requirements
from SanzyDev.database.variable import del_var, set_var
    ...
    if HOSTED_ON == 'heroku':
        if var.HEROKU_API_KEY is not None:
            ...
            if heroku_app is None:
                await eor(
                    xx,
                    f"{txt}\n**Kredensial Heroku tidak valid untuk deploy sanzy-Userbot dyno.**"
                )
                return repo.__del__()
            ...
            await eor(
                xx,
                "**✧ sanzy-userbot ✧** `Berhasil Di Update Userbot bisa di gunakan kembali.`"
            )
...
@sanzy_cmd(pattern="update( now| deploy|$)")
@register(
...
async def upstream(event):
    ...
    off_repo = b64decode(
        "aHR0cHM6Ly9naXRodWIuY29tL3Nhbnp5ZGV2L1Nhbnp5LVVzZXJib3Q="
    ).decode("utf-8")
    force_update = False
    try:
        repo = Repo()
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}update`\
        \n  »  **Kegunaan : **Untuk Melihat Pembaruan Terbaru Sanzy-Userbot.\
        \n\n  »  **Perintah :** `{cmd}update deploy`\
        \n  »  **Kegunaan : **Untuk MengUpdate Fitur Terbaru Dari Sanzy-Userbot.\
    "
    }
)
