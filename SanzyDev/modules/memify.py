...
from SanzyDev.sanzy import edit_delete, edit_or_reply, sanzy_cmd, runcmd, take_screen_shot

from . import cmd, var


@sanzy_cmd(pattern="mmf (.*)")
async def memify(event):
    ...
    input_file = await event.client.download_media(reply_msg, var.TEMP_DOWNLOAD_DIRECTORY)
    input_file = os.path.join(
        var.TEMP_DOWNLOAD_DIRECTORY,
        os.path.basename(input_file))
    if input_file.endswith(".tgs"):
        await xx.edit("**Mengekstrak Frame pertama...**")
        converted_file = os.path.join(var.TEMP_DOWNLOAD_DIRECTORY, "meme.webp")
        ...
        await xx.edit("**Mengekstrak Frame pertama...**")
        converted_file = os.path.join(var.TEMP_DOWNLOAD_DIRECTORY, "meme.png")
...
async def add_text_img(image_path, text):
    ...
    font = ImageFont.truetype(
        font="SanzyDev/sanzy/styles/FontLord.ttf",
    ...
    final_image = os.path.join(var.TEMP_DOWNLOAD_DIRECTORY, "memify.webp")
