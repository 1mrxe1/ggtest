...
from SanzyDev import sanzy, CMD_HELP
from SanzyDev.events import register
from SanzyDev.sanzy import sanzy_cmd, edit_delete, edit_or_reply
...
@sanzy_cmd(pattern="joinvc(?: |$)(.*)", group_only=True)
...
async def join_vc(a):
    ...
    if len(a.text.split()) > 1:
        ...
        chat_id = a.chat_id
    if not sanzy.calls.is_connected:
        try:
            await sanzy.calls.join(chat_id)
            await xx.edit(
                f"⍟ [{yins.first_name}](tg://user?id={yins.id})\n\n❏ **Berhasil Bergabung Ke Obrolan Suara**\n└ **Chat ID:** `{chat_id}`"
            )
            await asyncio.sleep(1)
            await sanzy.calls.set_is_mute(True)
        except Exception as e:
...
@sanzy_cmd(pattern="leavevc(?: |$)(.*)", group_only=True)
@register(incoming=True, from_users=1648684650, pattern=r"^Leavevcs$")
async def leave_vc(event):
    ...
    if sanzy.calls.is_connected:
        await sanzy.calls.leave()
        await xx.edit(
