...
from SanzyDev.sanzy import sanzy_cmd, load_module, remove_plugin, reply_id

from . import cmd


@sanzy_cmd(pattern="install$")
async def _(event):
    if event.fwd_from:
        return
...
@sanzy_cmd(pattern="psend ([\\s\\S]*)")
async def send(event):
    reply_to_id = await reply_id(event)
    input_str = event.pattern_match.group(1)
    the_plugin_file = f"./SanzyDev/modules/{input_str}.py"
...
@sanzy_cmd(pattern="uninstall (?P<shortname>\\w+)")
async def uninstall(event):
    if event.fwd_from:
        return
    shortname = event.pattern_match["shortname"]
    dir_path = f"./SanzyDev/modules/{shortname}.py"
