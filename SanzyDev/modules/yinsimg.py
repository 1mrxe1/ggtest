...
async def sketch(e):
    ...
    if yins.endswith(".tgs"):
        ...
        cmd = ["lottie_convert.py", yins, "sanzy.png"]
        file = "sanzy.png"
    ...
    cv2.imwrite("SanzyDev.png", pencil_sketch_IMG)
    await xx.respond("⍟ __Sketsa Hasilkan oleh sanzy-Userbot__ ⍟", file="SanzyDev.png")
    await xx.delete()
    os.remove(file)
    os.remove("SanzyDev.png")


@sanzy_cmd(pattern=r"grey(?: |$)(.*)")
async def xnxx(event):
    ...
    sanzy = await ureply.download_media()
    if sanzy.endswith(".tgs"):
        cmd = ["lottie_convert.py", sanzy, "yin.png"]
        file = "yin.png"
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
        ...
        img = cv2.VideoCapture(sanzy)
        heh, lol = img.read()
        cv2.imwrite("yin.png", lol)
        file = "yin.png"
    yin = cv2.imread(file)
    SanzyDev = cv2.cvtColor(yin, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("yin.jpg", SanzyDev)
...
@sanzy_cmd(pattern=r"blur(?: |$)(.*)")
async def sanzy(event):
    ureply = await event.get_reply_message()
    ...
    if yins.endswith(".tgs"):
        ...
        file = "yin.png"
    yin = cv2.imread(file)
    SanzyDev = cv2.GaussianBlur(yin, (35, 35), 0)
    cv2.imwrite("yin.jpg", SanzyDev)
    await event.client.send_file(
        event.chat_id,
        "yin.jpg",
...
@sanzy_cmd(pattern=r"negative(?: |$)(.*)")
async def yinsxd(event):
    ureply = await event.get_reply_message()
    sanzy = await eor(event, '**Memproses...**')
    if not (ureply and (ureply.media)):
        return await eod(sanzy, "`Bales Ke Gambar/Sticker`")

    SanzyDev = await ureply.download_media()
    if SanzyDev.endswith(".tgs"):
        cmd = ["lottie_convert.py", SanzyDev, "yin.png"]
        file = "yin.png"
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
        ...
        await sanzy.edit("**Sedang Menyiapkan Media...**")
        img = cv2.VideoCapture(SanzyDev)
        heh, lol = img.read()
        cv2.imwrite("yin.png", lol)
        file = "yin.png"
...
@sanzy_cmd(pattern=r"flp(?: |$)(.*)")
async def sanzy(kontol):
    ...
    SanzyDev = await ureply.download_media()
    if SanzyDev.endswith(".tgs"):
        cmd = ["lottie_convert.py", SanzyDev, "yins.png"]
        file = "yins.png"
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
...
@sanzy_cmd(pattern=r"quad(?: |$)(.*)")
async def sanzy(memek):
    ...
    SanzyDev = cv2.vconcat([mici, trn])
    cv2.imwrite("yins.jpg", SanzyDev)
...
async def yins(event):
    ...
    SanzyDev = asu.reshape(yins.shape)
    cv2.imwrite("yins.jpg", SanzyDev)
    await event.client.send_file(
        event.chat_id,
        "yins.jpg",
...
@sanzy_cmd(pattern=r"danger(?: |$)(.*)")
async def sanzy(event):
    ureply = await event.get_reply_message()
    sanzy = await eor(sanzy, '**Memproses...**')
    if not (ureply and (ureply.media)):
        return await eod(sanzy, "`Bales Ke Gambar/Sticker`")
...
async def pixelator(event):
    ...
    try:
        ...
        pass
    yins = await eor(event, '**Memproses...**')
    imsanzy = await reply_message.download_media()
    input_ = cv2.imread(imsanzy)
    height, width = input_.shape[:2]
    w, h = (hw, hw)
    temp = cv2.resize(input_, (w, h), interpolation=cv2.INTER_LINEAR)
    output = cv2.resize(temp, (width, height), interpolation=cv2.INTER_NEAREST)
    cv2.imwrite("output.jpg", output)
    await yins.respond("⍟ __Pixelator Dihasilkan oleh sanzy-Userbot__ ⍟", file="output.jpg")
    await yins.delete()
