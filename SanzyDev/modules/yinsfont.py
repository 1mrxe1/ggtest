...
@sanzy_cmd(pattern="font (.*)(|$)")
async def _(sanzy):
    font = sanzy.pattern_match.group(1)
    reply = await sanzy.get_reply_message()
    yins = None
    if not font:
        return await edit_delete(sanzy, f"<b>Silahkan berikan saya nama font...</b>\n\n<b>Gunakan <code>.lf</code> Untuk melihat daftar font</b>", time=5)
    if font == "smallcap":
    ...
    if yins is not None:
        await edit_or_reply(sanzy, yins)
