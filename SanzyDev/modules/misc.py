...
from SanzyDev.sanzy import sanzy_cmd, eor, time_formatter
...
@sanzy_cmd(pattern="sleep ([0-9]+)$", allow_sudo=False)
...
@sanzy_cmd(pattern="restart$", allow_sudo=False)
async def restart_bot(event):
    await eor(event, "**᯽ sanzy-userbot ᯽ Berhasil di Restart**")
    if var.BOTLOG_CHATID:
        await event.client.send_message(
            var.BOTLOG_CHATID,
            "#RESTART \n**᯽ sanzy-userbot ᯽ Berhasil Di Restart**"
        )
    args = [sys.executable, "-m", "SanzyDev"]
    execle(sys.executable, *args, environ)


@sanzy_cmd(pattern="repeat (.*)")
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}sleep`\
        \n  »  **Kegunaan : **Biarkan sanzy-Userbot tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Sanzy-Userbot`\
