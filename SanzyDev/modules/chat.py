...
from SanzyDev.sanzy import eod, eor
from SanzyDev.events import register
from SanzyDev.modules.yinsping import absen
from SanzyDev.sanzy import sanzy_cmd, get_user_from_event

from . import cmd


@sanzy_cmd(pattern="userid$")
...
@sanzy_cmd(pattern="inviteall ?(.*)")
async def get_users(event):
    sanzy_ = event.text[11:]
    chat_sanzy = sanzy_.lower()
    restricted = ["@sanzyChats", "@sanzychats", "@sanzyChat", "@sanzychat"]
    if chat_sanzy in restricted:
    ...
    sanzy = await eor(event, f"**Mengundang Member Dari Group {sanzy_}**")
    sanzyuserbot = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await sanzy.edit("**Tidak bisa Menambahkan Member di sini Harap ketik di Grup Chat**")
    s = 0
    f = 0
    error = "None"
    await sanzy.edit("**Terminal Status**\n\n`Sedang Mengumpulkan Pengguna...`")
    async for user in event.client.iter_participants(sanzyuserbot.full_chat.id):
        try:
...
CMD_HELP.update(
    {
        ...
        \n  •  **Contoh : **{cmd}link @sanzydevv baik\
    "
    }
)
