...
from SanzyDev.Sanzy import Sanzy_cmd, bash, eod, eor
...
@Sanzy_cmd(pattern="(ascii|asciis)$")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "**Mohon Balas Ke Media...**")
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        return await eod(event, "`Balas Ke Gambar/Sticker/Video`")
...
@Sanzy_cmd(pattern="asciibg(?: |$)(.*)")
async def _(event):
    BG = event.pattern_match.group(1)
    if BG.isnumeric():
        return await eor(event, "`Mohon Masukkan Warna Bukan Angka`")
    if not BG:
        return await eor(event, "`Mohon Masukkan Background Dari Ascii`")
    global bground
    bground = BG
    await eor(event, f"`Berhasil Setel Background Dari Ascii Ke` **{BG}**")
