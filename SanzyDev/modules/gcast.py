...
from SanzyDev.sanzy import sanzyChanger, sanzy_cmd, eod, eor
from SanzyDev.database.bl_gcast import add_gcast, cek_gcast, del_gcast
...
@sanzy_cmd(pattern="gcast(?: |$)(.*)")
async def gcast(event):
    me = await event.client.get_me()
    BLACKLIST_GCAST = sanzyChanger(cek_gcast(me.id))
...
@sanzy_cmd(pattern="addblacklist(?:\\s|$)([\\s\\S]*)")
async def add(event):
    me = await event.client.get_me()
    BLACKLIST_GCAST = sanzyChanger(cek_gcast(me.id))
...
@sanzy_cmd(pattern="delblacklist(?:\\s|$)([\\s\\S]*)")
async def _(event):
