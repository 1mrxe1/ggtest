...
from SanzyDev.sanzy import sanzy_cmd, bash, deEmojify
...
async def ngethreat(event):
    ...
    if replied.media:
        ...
        await event.edit("reply to a supported media file")
        return
    download_location = await sanzy.download_media(replied, var.TEMP_DOWNLOAD_DIRECTORY)
    ...
    await event.delete()
    await sanzy.send_file(event.chat_id, file, reply_to=replied)


@sanzy_cmd(pattern="trash(?: |$)(.*)")
...
async def ngetrap(e):
    ...
    await e.delete()
    await sanzy.send_file(e.chat_id, file, reply_to=replied)


# Ported by @AshSTR


@sanzy_cmd(pattern="fgs ((.*) ; (.*))")
async def FakeGoogleSearch(event):
    ...
    font1 = ImageFont.truetype(
        "SanzyDev/sanzy/styles/ProductSans-BoldItalic.ttf", 20)
    font2 = ImageFont.truetype(
        "SanzyDev/sanzy/styles/ProductSans-Light.ttf", 23)
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example  : **{cmd}tweet @sanzy (harus pake . [titik])\
    "
    }
)
